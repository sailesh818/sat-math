...
class AlgebraEasyPractisePage extends StatefulWidget {
  final String jsonFileName;
  final String title;

  const AlgebraEasyPractisePage({
    super.key,
  ...
  @override
  State<AlgebraEasyPractisePage> createState() =>
      _AlgebraEasyPractisePageState();
}

class _AlgebraEasyPractisePageState extends State<AlgebraEasyPractisePage> {
  ...
  Future<void> loadQuestions() async {
    try {
      final String response = await rootBundle.loadString(
          'assets/algebra/algebraeasy/${widget.jsonFileName}');
  ...
  void nextQuestion() async {
    if (currentQuestionIndex < questions.length - 1) {
      ...
      int basePoints = 4 + correctCount; // Easy level scoring
  ...
  void _loadRewardedAd() {
    RewardedAd.load(
      adUnitId: 'ca-app-pub-3940256099942544/5224354917', // Real ID ca-app-pub-6704136477020125/4913789019  // Test Ad ca-app-pub-3940256099942544/5224354917 
    ...
    _rewardedAd!.show(
      onUserEarnedReward: (ad, reward) async {
        if (forHint) {
          ...
          await savePointsToFirebase(4); // easy level reward
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("You earned 4 bonus points!")),
          );
        }
      },
    );
  ...
  Future<void> _useHint() async {
    ...
    if (totalPoints >= 2) {
      await savePointsToFirebase(-2); // cost for hint (easy)
      setState(() {
        showHint = true;
        hintPenalty += 2;
      });
      ...
      showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          content: const Text(
              "You need 2 reward points. Watch an ad to unlock this hint?"),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context),
              child: const Text("No, Thanks"),
