...
class CoordinateGeometryMediumPractisePage extends StatefulWidget {
  final String jsonFileName;
  final String title;

  const CoordinateGeometryMediumPractisePage({
    super.key,
  ...
  @override
  State<CoordinateGeometryMediumPractisePage> createState() =>
      _CoordinateGeometryMediumPractisePageState();
}

class _CoordinateGeometryMediumPractisePageState
    extends State<CoordinateGeometryMediumPractisePage> {
  ...
  void nextQuestion() async {
    if (currentQuestionIndex < questions.length - 1) {
      ...
      int basePoints = 6 + correctCount; // harder level = more points
    ...
    _rewardedAd!.show(
      onUserEarnedReward: (ad, reward) async {
        if (forHint) {
          ...
          await savePointsToFirebase(6); // medium level ad reward
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("You earned 6 reward points!")),
          );
        }
      },
    );
    ...
    if (totalPoints >= 3) {
      ...
      showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          content: const Text(
              "You need 3 reward points. Watch an ad to unlock this hint?"),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context),
              child: const Text("No, Thanks"),
