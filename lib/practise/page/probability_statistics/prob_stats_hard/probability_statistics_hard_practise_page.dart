...
class ProbabilityStatisticsHardPractisePage extends StatefulWidget {
  final String jsonFileName;
  final String title;

  const ProbabilityStatisticsHardPractisePage({
    super.key,
  ...
  @override
  State<ProbabilityStatisticsHardPractisePage> createState() =>
      _ProbabilityStatisticsHardPractisePageState();
}

class _ProbabilityStatisticsHardPractisePageState
    extends State<ProbabilityStatisticsHardPractisePage> {
  ...
  Future<void> loadQuestions() async {
    try {
      final String response = await rootBundle.loadString(
          'assets/probability/probabilityhard/${widget.jsonFileName}');
  ...
  void nextQuestion() async {
    if (currentQuestionIndex < questions.length - 1) {
      ...
      int basePoints = 10 + correctCount; // Hard level base points
  ...
  Future<void> _useHint() async {
    ...
    if (totalPoints >= 5) { // Hard hint costs 5 points
      await savePointsToFirebase(-5);
      setState(() {
        showHint = true;
        hintPenalty += 5;
      ...
      showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          content: const Text("You need 5 points. Watch an ad to unlock hint?"),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context),
